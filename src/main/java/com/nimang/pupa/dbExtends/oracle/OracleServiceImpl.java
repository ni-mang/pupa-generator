package com.nimang.pupa.dbExtends.oracle;
...
public class OracleServiceImpl implements IMetadataService {
    ...
    public String getUrl(ProDatasource datasource) {
        // jdbc:oracle:thin:@47.100.206.162:1521:xe
        String url = "jdbc:oracle:thin:@" + datasource.getMainAddr() +
    ...
    public List<ProTable> findTables(ProDatasource datasource, List<String> tableNames) {
        ...
        OracleMapper tbMapper = sqlSession.getMapper(OracleMapper.class);

        List<OracleTables> tablesList = tbMapper.queryTable(tableNames);
        sqlSession.close();
        ...
        for(OracleTables table:tablesList){
            ...
            proTable.setUpdateTime(LDTUtils.convertDateToLDT(table.getUpdateTime()));
    ...
    public List<ProField> findColumns(ProDatasource datasource, List<ProTable> proTableList) {
        ...
        List<OracleColumns> columnsList = mapper.queryColumns(tableNames);
        sqlSession.close();
        ...
        for(OracleColumns column:columnsList){
            ...
            proField.setRequiredFlag("N".equals(column.getIsNullable()));
    ...
    public void setBounds(ProField proField) {
        ...
        if (ObjectUtil.isNotNull(proField.getCharacterMaximumLength())) {
            // 字符串类型
            proField.setBoundMax(String.valueOf(proField.getCharacterMaximumLength()/3));
        }else {
            // 获取数值精度、刻度
